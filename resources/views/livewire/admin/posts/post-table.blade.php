<div>
    ...
    <x-slot name="header">
       <div class="flex mx-auto justify-items-stretch justify-between" role="group">
        ...
        <button type="button" class="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900">New post</button>
        </a>
       </div>
    </x-slot>
  <div class="isolate bg-white px-6 py-6 lg:px-8">
    ...
    <div class="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
      <table class="w-full text-left table-auto min-w-max">
        <thead>
          <tr>
              <th class="p-4 border-b border-slate-300 bg-slate-50"  wire:click="setSortFunctionality('title')">
              ...
              <th class="p-4 border-b border-slate-300 bg-slate-50" wire:click="setSortFunctionality('status')">
               <button class="flex items-center ml-1">
                   Status
        ...
        <tbody>
            @foreach ($posts as $item)
                <tr class="hover:bg-slate-50 border-b border-slate-200">
                    <td class="p-4 py-5 whitespace-nowrap text-sm font-medium text-gray-800">{{$item->title}} </td>
                    <td class="p-4 py-4 whitespace-nowrap text-sm text-gray-800">{{$item->status}}</td>
                    ...
                    <a href="{{ route('admin.posts.edit', $item) }}"><button type="button" class="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900">Edit</button></a><button type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" wire:click="deleteUser({{ $item->id }})"  wire:confirm="Are you sure you want to delete this product?">Delete</button>
                </div></td>
                </tr>
            @endforeach
        </tbody> 
